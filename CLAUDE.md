...
- **Collision is free.** Physics layers are defined on the TileSet, so tiles placed at runtime via `set_cell()` pick up collision automatically — no per-tile collision code.
- **Split into 13a (tech, hand-built Cathedral) / 13b (generation)** so traversal, collision, doors and dungeon encounters are debugged against a map whose layout is fully known, before randomness is layered on. Follows the existing 8a–8d / 19a–19d convention rather than renumbering the roadmap again.
...
| 13a | Dungeon tech (TileMapLayer, collision, doors/stairs, dungeon encounters, hand-built Cathedral) | **← next up** |
| 13b | Procedural dungeon generation (template → room graph → tile layout, seeded + persisted) | Not started |
