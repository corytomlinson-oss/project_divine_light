...
**Completed:** Milestone 11 — Formation & rows. Party members now have a `row` ("front"/"back"), defaulting to Vael+Ryn front / Lyra+Silas back. New "Swap Row" action added to the main battle menu (6 options now — Attack/Skill/Item/Defend/Swap Row/Run — the action menu scrolls to fit, reusing the same scroll system the Skill/Item menus already had); swapping is a queued action resolved in AGI turn order like Attack or Defend, not an instant menu shortcut. Back row reduces melee damage both dealt and received by 25% (`_row_mult()`), applied to every physical/melee-classified formula; magic and Ryn's Ki Blast (explicitly "ranged") are unaffected by row. This unblocks the five skills that were scoped row-independent back in Milestones 8c/8d: **Shadow Strike** now requires the front row to even appear in Silas's skill menu; **Vanish** now actually moves Silas to the back row (not just a flavor no-op); **Tremor** shows as "Tremor" (single-target) from the front row or "Tremor (AoE)" (reduced damage, hits all enemies) from the back row, computed live off Lyra's current row when the skill menu opens; **Ki Blast** is flagged `"ranged": true` so it deals full damage regardless of Ryn's row; **Divine Shield** now only buffs allies sharing Vael's row, not the whole party. Party HP labels show a compact row tag (F/B) and the turn-selection header shows the full row name. **Scoped out:** the outside-battle Formation menu (README's "Select: Formation menu") wasn't built — there's no overworld menu system at all yet (Player.gd only handles movement), so row swaps are battle-only for now, at the cost of a turn like the design doc specifies for in-battle swaps.
...
**Completed:** Milestone 13a — Dungeon tech. `Player` was extracted from Overworld into its own reusable scene (`scenes/player/Player.tscn`) so Dungeon scenes can instance the same movement/collision code instead of duplicating it. Wall collision and doors/stairs are both grid-based, not physics-based — `Player.gd` checks the destination tile's atlas coordinates before allowing a move (`(1,0)` = wall, blocked) and after completing a move (`(2,0)` = door/stairs, triggers a scene transition looked up from the current map's own `get_door_destination(cell)`). The placeholder tileset grew a third tile (gold, 48×16 now) for this. `GameManager` gained `current_location`/`current_scene_path`, set by each map scene's own `_ready()`, so `Battle.gd` knows which enemy table to draw from (`ENCOUNTER_TABLES`, keyed by location) and which scene to return to after battle/escape — replacing two places that used to hardcode `Overworld.tscn`. The Cathedral got one hand-built test room (`scenes/dungeon/CathedralDungeon.tscn` + `scripts/dungeon/Dungeon.gd`, walled border, one entrance/exit door) with a placeholder stat-only enemy table (Fallen Priest/Cursed Paladin/Shadow Acolyte) — their README-described abilities (DEF debuff, stun, buff allies) need an enemy-ability system that doesn't exist yet, so that's deferred to Milestone 18a. **Discovered along the way:** the Milestone 1 hand-painted Overworld map has zero wall tiles anywhere (259 floor tiles, no walls) — the wall tile type was registered but never painted, so overworld collision is a no-op today even though the underlying mechanism is shared and fully working (proven by the Cathedral's walled room). Also found during playtesting: returning from a battle used to always reset the player to the map's fixed spawn point instead of where the encounter actually triggered (pre-existing since Milestone 2, just newly noticeable). Fixed via `GameManager.pending_spawn_position`/`has_pending_spawn`, stashed right before entering battle and consumed on the next map load — covers victory, defeat, and successful escape, in both the overworld and dungeons, without affecting door transitions. Verified headlessly — see CLAUDE.md's Milestone 13a section for how, since `--script`-mode testing needed a workaround for autoloads.

**Completed:** Milestone 13b — Procedural dungeon generation. `DungeonGenerator` (pure logic, no scene dependencies — `scripts/dungeon/DungeonGenerator.gd`) replaces 13a's fixed test room with the three-layer approach from the design decision: a hand-authored **template** (room count range, room size range) drives a procedural **room graph** — a random tree where every parent-child pair sits exactly one grid step apart, so every room is reachable by construction and corridors never need to route around a third room — which drives a procedural **tile layout** (rooms centered in a slot grid, corridors carved between centers, then a wall shell added automatically around every exposed floor edge). Anchors: entrance = tree root, boss = deepest room, captive = the midpoint of the entrance→boss path (or any spare room if that path has no midpoint) — both entrance and boss rooms get a door tile (either leads back to the overworld, same dual-exit pattern as 13a), captive gets a new 4th placeholder tile (violet) marking its room, purely for testability, since there's no rescue mechanic yet. Seeds are lazily rolled once per dungeon (`GameManager.get_dungeon_seed()`, using the global RNG for that one *pick* only — never for the generation itself) and now persist through `save_game()`/`load_game()` alongside party/inventory, so re-entering a dungeon — even in a save loaded fresh — reproduces the exact same layout. `Player.snap_to()` places the player at the generated spawn point directly. **Fixed during playtesting:** returning from a battle fought away from the entrance was resetting the player back to the entrance instead of where the fight happened — which also made other rooms look unreachable, since getting yanked back after every fight meant never walking far enough uninterrupted to actually reach them. Both map controllers (`Overworld.gd`/`Dungeon.gd`) now decide for themselves, in their own `_ready()`, whether to restore a stashed battle-return position or use their own default/generated spawn — previously `Player.gd` tried to make that call itself, but its `_ready()` runs before its parent map's, so the map's own spawn logic was silently clobbering the restored position right after. Verified headlessly across 200+ seeds: room count always within template range, entrance/exit/captive always distinct and correctly tagged, every room reachable from the entrance (confirmed with a dedicated 500-seed sweep after this bug surfaced), wall shell always sealed with no gaps, same-seed regeneration always byte-identical, and battle-return positions now survive a full scene reload in both the overworld and the dungeon. **Also fixed:** the camera was pinning the player to the bottom-right corner of the screen instead of centering them — a Milestone 1 leftover offset that only mattered once navigating actually depended on seeing your surroundings in all directions, which a maze needs and the open overworld never demanded. Player's `Camera2D` now centers correctly.

**Completed:** Milestone 14 — Boss encounters. Bosses are **visible, fixed encounters** rather than random ones — a new placeholder tile (crimson, tileset now 80×16) placed by `DungeonGenerator` just inside the boss room's door (same offset pattern as the player's dungeon-entry spawn), and walking onto it always starts the fight, no step-counter roll involved. **Multi-phase:** `Combatant` gained `is_boss`/`boss_phase`/`phase_hp_thresholds`; `Battle.gd` checks the boss's HP fraction once per its own turn (not the instant a threshold is crossed, so it reads as "recoils, then comes back stronger" rather than interrupting whatever just hit it) and on crossing a threshold permanently raises its ATK and, from then on, layers a DEF-down debuff onto whichever party member it hits — real per-phase skill kits (the README's boss-specific abilities like the Fallen Guardian's corrupted holy magic) need an enemy-ability dispatch system that doesn't exist yet, so this milestone proves the phase *mechanism*, not final boss content. **Escape is blocked** in any battle with a boss enemy present. **Boss XP is a flat large number** (300 for the test boss) rather than a formula — "should push the party at least one level" is satisfied by the number itself, no new system needed. **Defeated bosses stay defeated:** `GameManager.defeated_bosses` (now persisted through save/load alongside dungeon seeds) reverts the trigger tile to plain floor on the next visit, so a beaten boss can't be re-farmed. The Cathedral's boss room now hosts a placeholder test boss ("Hollow Warden") — clearly not the real Fallen Guardian, which is Milestone 18a's job once Act I content exists. Verified headlessly: 300-seed sweep confirming the boss trigger is always distinct from the other anchors, correctly tagged, and reachable; boss encounter setup (single enemy, `is_boss` flag, correct stats); escape lockout firing only in boss battles; phase transition (ATK increase + debuff application, and that it doesn't re-trigger past its only threshold); victory marking the boss defeated and that surviving a save/load round-trip; and the dungeon repainting a defeated boss's tile as floor on the next load.

**Roadmap restructured (2026-08-06):** what used to be milestones 15 (Sprites & tiles) and 16 (Music & sound) — one monolithic art pass and one monolithic audio pass, done all at once late in the project — are now split into per-content art/music passes so art gets generated section by section as content is actually written, instead of waiting until everything exists. See CLAUDE.md's Roadmap restructuring section for the full reasoning and the new milestone numbers (15–25).

**Completed:** Milestone 15 — Equipment & gear. Five slots per character (Weapon/Armor/Helmet/Gloves/Accessory) on a new `Combatant.equipment: Dictionary`. Stat bonuses from gear are applied by directly adding/subtracting from the character's own `atk`/`defense`/`int_stat`/`res_stat`/`agi`/`max_hp`/`max_mp` fields on equip/unequip (`scripts/systems/Equipment.gd`, new `class_name Equipment`) rather than a separate bonus layer — every existing combat formula already reads those fields, so gear flows through for free without touching ~40 call sites, the same way level-up gains already do. Weapons/Armor/Helmet/Gloves are class-locked; Accessories are universal, per the design doc. **Full Set Bonus:** `Equipment.has_set_bonus()` checks whether Armor+Helmet+Gloves+Accessory are all equipped and share a `set_id` — one real bonus is wired up end-to-end (Vael's Holy Guardian Set, "all buff skills last 1 extra round," now actually extends Guard/Fortify/Divine Shield/Battle Hymn from 2 to 3 rounds); the other 11 sets from the design doc are documented but not yet mechanically implemented, since there's no real loot to award them through until Act I/II content exists — add each as its own set gets used, same pattern as this one. **New equip screen** (`scenes/equip/Equip.tscn`) — the game's first overworld-accessible UI, opened with **E** from `Player.gd` (works from both the overworld and dungeons, since Player.gd is already shared): character select → slot list → item picker, all list-based like Battle's existing menus. Deliberately *not* the full "B = main menu" shell from the design doc's controller mapping — Formation/Inventory/Party-status screens don't exist yet either, so this is a single-purpose key straight to the one screen that exists. Starter placeholder gear (one weapon per class, Vael's full Holy Guardian Set, one universal accessory) seeded into `GameManager.inventory` so the system is testable without a shop/loot system, which doesn't exist yet. Equipment now round-trips through save/load alongside everything else. Verified headlessly (37 checks): stat math on equip/unequip including re-equip not double-stacking, class-lock enforcement, universal-accessory behavior, set-bonus detection at each of the 4 pieces, the buff-duration bonus actually changing Battle.gd's behavior (and not leaking to other characters), inventory filtering, save/load round-trip, and the equip screen's full navigation flow (character → slot → item → equip → back). **Fixed after playtesting:** the equip screen only ever announced the Holy Guardian Set once fully complete, with no indication of partial progress — easy to end up 1 piece short (especially the universal accessory, which can land on the wrong character by accident) without any explanation why the bonus isn't kicking in. It now shows live progress ("Holy Guardian Set: 3/4 equipped") whenever at least one piece is on, and tags set pieces in the item list with their set name so it's clear which items belong together.

**Next milestone:** Milestone 16 — Current-content art & music pass
...
**Split into 13a/13b** (matching the 8a–8d / 18a–18d convention) so dungeon traversal tech is proven against a fixed, hand-built map before generation randomness is layered on top.
...
| 14 | **Boss encounters** | Visible on-screen enemies, multi-phase boss fights — a generic system built here, then used by both Act I's dungeon bosses (18a–18d) and Act II's kingdom bosses (22b–22d) |
| 15 | **Equipment & gear** | Real equipment system — 5 slots per character (Weapon/Armor/Helmet/Gloves/Accessory), class-locked weapons/armor, stat bonuses from gear, Full Set Bonus tracking (3 sets per class, one per act, passive skill upgrades), and an equip UI screen (doesn't exist yet — nothing outside the battle scene currently handles menu input). Deliberately placed after dungeon-map tech (13) exists so loot has somewhere to drop from, and before Act I content so the system is ready when real armor-set loot gets placed in dungeons/shops. Built with placeholder art like everything so far — doesn't wait on 16 |
| 16 | **Current-content art & music pass** | First real (non-placeholder) art/audio pass, covering everything built so far: Player sprite, Overworld tileset, the Cathedral's generated-dungeon tileset (floor/wall/door/captive/boss — the same 5-tile vocabulary `DungeonGenerator` paints with), current enemy sprites (overworld's four + the Cathedral's three placeholder enemies + the Milestone 14 test boss), Battle screen backgrounds/UI frames, and baseline BGM (overworld, battle) + SFX (attack, menu, victory, level-up). **Why here, not one monolithic art pass at the very end:** splits what used to be milestones 15/16 (Sprites & tiles, Music & sound) to mirror content delivery instead — see the Roadmap restructuring section in CLAUDE.md for the full reasoning. **Known tradeoff:** the Cathedral's placeholder enemies and test boss get real art here but may be replaced/renamed once Act I content (18a) delivers the real Fallen Guardian and real enemy roster — accepted deliberately so there's something to actually look at now rather than waiting. **Asset strategy:** source pixel art packs from itch.io or Kenney.nl first, Midjourney/DALL-E for concept generation if needed (prompts written per asset); Suno AI (suno.com) for chiptune/SNES-style BGM, OpenGameArt.org for pre-made free tracks |
| 17 | **UI/UX polish** | Once the current-content art/music pass (16) exists, do a design pass over the UI built so far — HUD composition (party panel, enemy panel, menus), screen transition/animation polish, spacing/readability, controller-nav feel — using real assets to design around instead of placeholder colors. Doesn't need to wait for *every* asset in the whole game to exist first, just enough real material to work with. Explicitly does **not** include Equipment's equip screen (15) or Save/load's slot-picker screen (20) — those are new screens with their own functional scope, built when their milestone comes up |
| 18a–18d | **Act I content** | One per dungeon (mirrors the 8a–8d Class Skills pattern — each is a working, testable slice: dungeon → boss → rescue). See breakdown below |
| 19 | **Act I art & music pass** | Real art/audio for everything Act I added: 4 dungeon tilesets with their own identity, 4 boss sprites (Fallen Guardian, Corrupted Grandmaster, Void Archmage, Shadow Lord), Frank + the 3 rescued party members' sprites/portraits, Verdance + Edenmere town tilesets/NPCs, Act I's real enemy roster, dungeon/boss/town BGM + SFX |
| 20 | **Save / load — triggers & integration** | Wires the Milestone 12 core system to real triggers: manual save at inns (3 slots), auto-save after boss defeats/rescues/region clears, suspend save on app close including mid-dungeon position. Placed here deliberately — none of these triggers exist until Act I's towns, dungeons, and bosses do |
| 21 | **Android APK export** | Build and deploy to RP6, test controller input. First real on-hardware checkpoint — by this point Act I is content-complete *and* has real art/audio (18a–18d, then 19), so this is the first build that actually looks and sounds like the finished game, not a placeholder slice |
| 22a–22d | **Act II content** | A shared cleansing-transformation system plus one sub-milestone per kingdom. See breakdown below |
| 23 | **Act II art & music pass** | Real art/audio for Act II: 3 dungeon tilesets with their unique mechanics visualized (Frozen Citadel's frozen chests, Sunken Sanctum's fire hazards, Drowned Fortress's flooded corridors), 3 mini-bosses + 3 kingdom bosses (3-phase — may want distinct visuals per phase), each kingdom town's before/after cleansing states, Act II's enemy roster, dungeon/boss/town BGM + SFX |
| 24 | **Act III + Vorath** | The Blighted Maw, 3-phase final boss + Frank's revelation |
| 25 | **Act III art & music pass** | Real art/audio for the finale: The Blighted Maw, Vorath's 3-phase final boss (including Void Form), Frank's true-identity reveal, Act III's enemy roster, final dungeon/boss BGM, any ending sequence audio |
...
### Milestone 18 — Act I Breakdown

| # | Sub-milestone | Description |
|---|---|---|
| 18a | The Cathedral (Vael) | Verdance town (initial state) + starting-class routing foundation, Cathedral dungeon, solo-escape warden variant (used if Vael is the starting class), Fallen Guardian boss, Vael rescue/recruit |
| 18b | The Monastery (Ryn) | Monastery dungeon, solo-escape warden variant (if Ryn is starting class), Corrupted Grandmaster boss, Ryn rescue/recruit |
| 18c | The Observatory (Lyra) | Observatory dungeon, solo-escape warden variant (if Lyra is starting class), Void Archmage boss, Lyra rescue/recruit |
| 18d | The Underground Guild (Silas) | Underground Guild dungeon, solo-escape warden variant (if Silas is starting class), Shadow Lord boss, Silas rescue/recruit, Edenmere town, Act I → Act II transition |

### Milestone 22 — Act II Breakdown

| # | Sub-milestone | Description |
|---|---|---|
| 22a | Cleansing transformation system | Generic 3-layer transformation infrastructure (overworld visuals / town NPC+shop state / gameplay unlocks), built once and shared by all 3 kingdoms rather than built ad hoc three times |
| 22b | Nordveil (Tundra) | Frozen Citadel dungeon (frozen-chest fire-thaw mechanic), Frost Warden mini-boss, Frozen Sovereign 3-phase boss, Nordveil before/after cleansing |
| 22c | Duskara (Desert) | Sunken Sanctum dungeon (fire hazard tiles), Blazing Sentinel mini-boss, Burning Tyrant 3-phase boss, Duskara before/after cleansing |
| 22d | Selavon (Coastal) | Drowned Fortress dungeon (flooded corridors), Admiral's First Mate mini-boss, Drowned Admiral 3-phase boss, Selavon before/after cleansing |
