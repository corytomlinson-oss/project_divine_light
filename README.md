...
| 17 | **UI/UX polish** | Once real sprites/tilesets/UI frames exist (15) and BGM/SFX exist (16), do one real design pass over the whole game's UI/UX rather than the ad hoc placeholder layouts built milestone-by-milestone so far: HUD composition (party panel, enemy panel, menus), screen transition/animation polish, spacing/readability, and controller-nav feel end to end. Explicitly does **not** include Equipment's equip screen (18) or Save/load's slot-picker screen (20) — those are new screens with their own functional scope, built when their milestone comes up; this milestone is about making everything already built (and those two once they exist) look and feel like one cohesive game instead of a stack of placeholder-era screens |
...
| 19 | **Act I content** | Split into 19a–19d, one per dungeon (mirrors the 8a–8d Class Skills pattern — each is a working, testable slice: dungeon → boss → rescue). See breakdown below |
...
| # | Sub-milestone | Description |
|---|---|---|
| 19a | The Cathedral (Vael) | Verdance town (initial state) + starting-class routing foundation, Cathedral dungeon, solo-escape warden variant (used if Vael is the starting class), Fallen Guardian boss, Vael rescue/recruit |
| 19b | The Monastery (Ryn) | Monastery dungeon, solo-escape warden variant (if Ryn is starting class), Corrupted Grandmaster boss, Ryn rescue/recruit |
| 19c | The Observatory (Lyra) | Observatory dungeon, solo-escape warden variant (if Lyra is starting class), Void Archmage boss, Lyra rescue/recruit |
| 19d | The Underground Guild (Silas) | Underground Guild dungeon, solo-escape warden variant (if Silas is starting class), Shadow Lord boss, Silas rescue/recruit, Edenmere town, Act I → Act II transition |
