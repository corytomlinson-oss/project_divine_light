...
| 12 | **Save / load — core system** | Save file format + serialization for `GameManager` state (party, inventory, formation). Debug-triggerable save/load — a real save-slot UI comes later, in Milestone 19. Buildable now since it only depends on data structures that already exist, not on any in-game trigger |
...
| 14 | **Boss encounters** | Visible on-screen enemies, multi-phase boss fights — a generic system built here, then used by both Act I's dungeon bosses (18) and Act II's kingdom bosses (21) |
...
| 17 | **Equipment & gear** | Real equipment system — 5 slots per character (Weapon/Armor/Helmet/Gloves/Accessory), class-locked weapons/armor, stat bonuses from gear, Full Set Bonus tracking (3 sets per class, one per act, passive skill upgrades), and an equip UI screen (doesn't exist yet — nothing outside the battle scene currently handles menu input). Deliberately placed after dungeon-map tech (13) exists so loot has somewhere to drop from, and just before Act I content so the system is ready when real armor-set loot gets placed in dungeons/shops |
| 18 | **Act I content** | Split into 18a–18d, one per dungeon (mirrors the 8a–8d Class Skills pattern — each is a working, testable slice: dungeon → boss → rescue). See breakdown below |
| 19 | **Save / load — triggers & integration** | Wires the Milestone 12 core system to real triggers: manual save at inns (3 slots), auto-save after boss defeats/rescues/region clears, suspend save on app close including mid-dungeon position. Placed here deliberately — none of these triggers exist until Act I's towns, dungeons, and bosses do |
| 20 | **Android APK export** | Build and deploy to RP6, test controller input |
| 21 | **Act II content** | Split into 21a–21d — a shared cleansing-transformation system plus one sub-milestone per kingdom. See breakdown below |
...
### Milestone 21 — Act II Breakdown

| # | Sub-milestone | Description |
|---|---|---|
| 21a | Cleansing transformation system | Generic 3-layer transformation infrastructure (overworld visuals / town NPC+shop state / gameplay unlocks), built once and shared by all 3 kingdoms rather than built ad hoc three times |
| 21b | Nordveil (Tundra) | Frozen Citadel dungeon (frozen-chest fire-thaw mechanic), Frost Warden mini-boss, Frozen Sovereign 3-phase boss, Nordveil before/after cleansing |
| 21c | Duskara (Desert) | Sunken Sanctum dungeon (fire hazard tiles), Blazing Sentinel mini-boss, Burning Tyrant 3-phase boss, Duskara before/after cleansing |
| 21d | Selavon (Coastal) | Drowned Fortress dungeon (flooded corridors), Admiral's First Mate mini-boss, Drowned Admiral 3-phase boss, Selavon before/after cleansing |
